...
class DatabaseHandler:
    ...
    def execute_expression(self, sql_expression: str, sql_parameters: tuple):
        with psycopg2.connect(
            dbname=self.conf["DATABASE_CONNECTION_INFO"]["DB_NAME"],
            ...
            port=self.conf["DATABASE_CONNECTION_INFO"]["DB_PORT"],
        ) as conn:
    ...
    def insert_site(self, site_name: str) -> Site:
        ...
        site_id = self.execute_expression(
            sql_expression=sql_expression, sql_parameters=sql_parameters
        )[0][0]
    ...
    def insert_node(self, lat: float, lon: float, site_id: int) -> Node:
        ...
        node_id = self.execute_expression(
            sql_expression=sql_expression, sql_parameters=sql_parameters
        )[0][0]
    ...
    def insert_species(self, species_name) -> Species:
        ...
        species_id = self.execute_expression(
            sql_expression=sql_expression, sql_parameters=sql_parameters
        )[0][0]
    ...
    def insert_plant(self, plant: Plant) -> Plant:
        ...
        plant_id = self.execute_expression(
            sql_expression=sql_expression, sql_parameters=sql_parameters
        )[0][0]
    ...
    def insert_sensordata(self, sensordata_obj: SensorData) -> SensorData:
        ...
        sensor_data_id = self.execute_expression(
            sql_expression=sql_expression, sql_parameters=sql_parameters
        )[0][0]
    ...
    def insert_node_to_node_link(
        self, node_1_id: int, node_2_id: int, weight: float
    ) -> None:
        sql_expression = """
    ...
    def get_all_sites(self):
        ...
        sites = self.execute_expression(
            sql_expression=sql_expression, sql_parameters=()
        )
    ...
    def get_sensordata(self, node_id: int) -> [SensorData]:
        ...
        sensordata = self.execute_expression(
            sql_expression=sql_expression, sql_parameters=sql_parameters
        )
    ...
    def get_plants(self, node_id: int) -> [Plant]:
        ...
        plants = self.execute_expression(
            sql_expression=sql_expression, sql_parameters=sql_parameters
        )

        plants = [
            Plant(plant_id=x[0], species_id=x[1], is_diseased=x[2], date_recorded=x[3])
            for x in plants
        ]
