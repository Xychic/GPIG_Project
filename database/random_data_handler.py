...
class RandomDataHandler:
    ...
    def generate_random_sensordata(self):
        ...
        return SensorData(co2_level=round(random.uniform(300, 2200), 3),
    ...
    def generate_site_and_nodes(self, site_name, node_count):
        ...
        for i in range(node_count):
            new_node = self.database_handler.insert_node(lat=round(random.uniform(lat_min, lat_max), 4),
    ...
    def simulate_live_data_insertion(self, site_name: str, node_count: int = 20):
        ...
        while True:
            ...
            if random.uniform(0, 1) > 0.8:
                ...
                self.database_handler.insert_node_to_data_link(int(node_to_use.id), new_sensordata.id)
    ...
    def generate_populated_site(self, site_name: str, node_count: int = 20, sensordata_per_node: int = 5):
        ...
        for node in nodes:
            ...
            for i in range(sensordata_per_node):
                ...
                self.database_handler.insert_node_to_data_link(int(node.id), new_sensordata.id)
